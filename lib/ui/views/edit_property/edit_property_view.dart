...
import 'package:real_estate_fe/ui/views/edit_property/edit_property_viewmodel.dart';
...
class EditPropertyView extends StackedView<EditPropertyViewModel> {
  final Property property;

  const EditPropertyView({required this.property, Key? key}) : super(key: key);

  @override
  Widget builder(
      BuildContext context, EditPropertyViewModel viewModel, Widget? child) {
    return BasePage(
      showAppBar: true,
      title: "Edit Property",
  ...
  Widget _buildBasicInfoStep(
      BuildContext context, EditPropertyViewModel viewModel) {
...
// _buildReviewStep function updated with summary view
  Widget _buildReviewStep(EditPropertyViewModel viewModel) {
      ...
      child: Column(
        ...
        children: [
          ...
          GridView.builder(
            ...
            itemCount: viewModel.allImages.length,
            itemBuilder: (context, index) {
              return _buildImageThumbnail(viewModel, index);
            },
          ),
          'Submit Listing'
...
// _buildSummary function
  Widget _buildSummary(EditPropertyViewModel viewModel) {
    return VStack(
      [
        ...
        _buildSummaryItem(
            'Address', (viewModel.selectedLocation.address ?? "").toString()),
      ],
    ).py16().px12();
  }
...
// _buildSummaryItem function
  ...
  Widget _buildNavigationControls(EditPropertyViewModel viewModel) {
  ...
  _buildDescriptionStep(BuildContext context, EditPropertyViewModel viewModel) {
    return VStack([
  ...
  Widget _buildMediaStep(EditPropertyViewModel viewModel) {
    return VStack([
      ...
      GridView.builder(
        ...
        itemCount: viewModel.allImages.length + 1,
        itemBuilder: (context, index) {
          if (index == viewModel.allImages.length) {
  ...
  Widget _buildImageThumbnail(EditPropertyViewModel viewModel, int index) {
    final image = viewModel.allImages[index];
    return Stack(
      children: [
        ...
        Positioned(
          ...
          right: -10,
          child: IconButton(
            icon: const Icon(Icons.close, color: Vx.red800, size: 20),
            onPressed: () => viewModel.handleImageRemove(index),
          ),
        ),
      ],
    ).cornerRadius(5);
  ...
  @override
  EditPropertyViewModel viewModelBuilder(BuildContext context) =>
      EditPropertyViewModel(property: property);
  @override
  void onViewModelReady(EditPropertyViewModel viewModel) {
